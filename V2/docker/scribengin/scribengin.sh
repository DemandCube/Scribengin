...
function build(){
  ...
  #Release the project
  cd $DOCKERSCRIBEDIR/../../scribengin/release
...
function deploy(){
  ...
  scp -q -o StrictHostKeyChecking=no -r $DOCKERSCRIBEDIR/../../scribengin/release/build/release neverwinterdp@hadoop-master:/opt/scribengin
