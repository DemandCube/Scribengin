...
public class JAccordionPanel extends JPanel implements ActionListener {
  /**
  ...
  /**
   * A LinkedHashMap of bars: we use a linked hash map to preserve the order of
   * the bars
   */
  ...
  /**
   * Programmatically sets the currently visible bar; the visible bar index must
   * be in the range of 0 to size() - 1
   * 
   * @param visibleBar
   ...
   * rebuilds the top and bottom panels of bars as well as making the currently
   * selected bar's panel visible
   */
  public void render() {
    ...
    // hold to correct frequency of bars, add the bars, and "validate" it to
  ...
  public static void main(String[] args) {
    JFrame frame = new JFrame("JAccordionPanel Test");
    JAccordionPanel accordionPanel = new JAccordionPanel();
    accordionPanel.addBar("One", getDummyPanel("One"));
    accordionPanel.addBar("Two", getDummyPanel("Two"));
    accordionPanel.addBar("Three", getDummyPanel("Three"));
    accordionPanel.addBar("Four", getDummyPanel("Four"));
    accordionPanel.addBar("Five", getDummyPanel("Five"));
    accordionPanel.setVisibleBar(2);
    frame.getContentPane().add(accordionPanel);
