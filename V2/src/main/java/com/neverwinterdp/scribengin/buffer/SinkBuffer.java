...
public final class SinkBuffer {
  ...
  public SinkBuffer(SinkPartitioner partitioner, S3SinkConfig config) {
    ...
    bufferThread.start();
    logger = Logger.getLogger(SinkBuffer.class);
  }
    ...
    try {
      ...
      if (tuplesChunk.size() == chunkSize) {
        try {
          ...
          // call partitioner to get the path of the file depending on the offset
          ...
          for (Tuple t : tuplesChunk) {
            MappedByteBuffer mem =
                fileChannel.map(FileChannel.MapMode.READ_WRITE, 0, mappedByteBufferSize);
            if (!mem.hasRemaining()) {
              start += mem.position();
              mem = fileChannel.map(FileChannel.MapMode.READ_WRITE, start, t.getData().length);
            }
            mem.put(t.getData());
          }
  ...
  /**
   * Writes all tuples to disk creating more in-memory space.
   * A few tuples that couldn't complete a chunk will be left in-memory.
   */
  ...
  private void updateDiskState() {
    ...
    /*
     * if (files.size() > maxTuplesOnDisk || getTuplesSizeOnDisk() > maxBufferSizeOnDisk
  ...
  private boolean getIsSaturated() {
    return files.size() > maxTuplesOnDisk || getTuplesSizeOnDisk() > maxBufferSizeOnDisk
        || (startBufferingTimeOnDisk != 0
        && (System.currentTimeMillis() - startBufferingTimeOnDisk) > maxBufferingTimeOnDisk);
  }

  /*
   * Note that there are other methods for reading on-File size vs in-Memory sizes
   */
  public int size() {
